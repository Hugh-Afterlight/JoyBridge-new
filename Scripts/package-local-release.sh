...
DERIVED_DATA_DIR="${DERIVED_DATA_DIR:-/private/tmp/JoyBridgeNewPackageDerivedData}"
STAGING_DIR="/private/tmp/JoyBridgeNewPackage-$SAFE_VERSION"
...
echo "==> JoyBridge-new local package"
...
{
  printf '%s\n' "JoyBridge-new local test package"
  printf '%s\n' "Version: $VERSION"
  ...
  printf '%s\n' "- Recommended order: move JoyBridge-new.app to /Applications first, then open it, then grant Accessibility permission."
  ...
  printf '%s\n' "- Local tester fallback, only for builds you trust: xattr -dr com.apple.quarantine /Applications/JoyBridge-new.app"
  printf '%s\n' "- Open the app manually after each restart. Login item autostart is not implemented yet."
  printf '%s\n' "- Grant Accessibility permission to this installed copy of JoyBridge-new before testing mappings."
  ...
  printf '%s\n' "- 建议顺序：先把 JoyBridge-new.app 移到“应用程序”，再打开，再授权辅助功能权限。"
  printf '%s\n' "- 如果 macOS 提示 Apple 无法验证 JoyBridge-new，请点完成，再打开 系统设置 > 隐私与安全性 > 安全性，点击仍要打开。"
  printf '%s\n' "- Apple 通常只会在你尝试打开 App 后约一小时内显示仍要打开按钮。"
  printf '%s\n' "- 本地测试者备用命令，仅用于你信任的测试包：xattr -dr com.apple.quarantine /Applications/JoyBridge-new.app"
  printf '%s\n' "- Mac 重启后仍需要手动打开 JoyBridge-new，暂时没有开机自启。"
  printf '%s\n' "- 测试映射前，请给这个安装后的 JoyBridge-new 授权辅助功能权限。"
